...
function rendreNote() {
  ...
  if (!dispo) {
    c.innerHTML =
      ...
      'de deux mille à douze mille jetons d\'entrée.</p>' +
...
/* `texteProposition()` et `signatureProposition()` ont déménagé dans
   `js/ui/dossier.js`. Ce ne sont pas des vues : ce sont ce que le dossier
   VAUT, mis en lignes. Le déménagement les rend exécutables en Node — la
   tâche planifiée qui envoie la proposition par e-mail les appelle, et
   deux versions du même message finiraient par diverger. */
